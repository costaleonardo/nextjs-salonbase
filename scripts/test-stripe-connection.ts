...
async function testStripeConnection() {
  ...
  try {
    ...
    console.log(
      `   Available balance: ${account.available.map((b) => `${b.amount / 100} ${b.currency.toUpperCase()}`).join(", ")}`
    );
    console.log(
      `   Pending balance: ${account.pending.map((b) => `${b.amount / 100} ${b.currency.toUpperCase()}`).join(", ")}\n`
    );
