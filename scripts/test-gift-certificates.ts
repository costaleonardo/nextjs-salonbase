...
async function testGiftCertificateDeduplication() {
  ...
  try {
    ...
    if (testCodes.size < iterations) {
      console.log(`⚠️  ${iterations - testCodes.size} duplicate codes were generated (expected with random generation)`)
    }
    ...
    try {
      ...
      console.log('❌ FAILED: Duplicate certificate was created (uniqueness constraint not working)')
    } catch (error) {
    ...
    if (parseFloat(updated.balance.toString()) === parseFloat(cert1.balance.toString()) - redeemAmount) {
