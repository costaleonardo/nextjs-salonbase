...
async function testAuditTrailCompleteness() {
  ...
  try {
    ...
    if (missingEvents === 0) {
      console.log(`✅ PASSED: All ${completedPayments.length} completed payments have required events\n`)
    } else {
    ...
    for (const payment of failedPayments) {
      const hasRollback = payment.auditLogs.some(log =>
        log.action === 'payment_rolled_back' || log.action.includes('failed')
      )
    ...
    for (const payment of refundedPayments) {
      const hasRefundLog = payment.auditLogs.some(log =>
    ...
    for (const payment of giftCertPayments) {
      const hasCertLog = payment.auditLogs.some(log =>
        log.action.includes('gift_certificate')
      )
    ...
    if (missingCertLogs === 0) {
      console.log(`✅ PASSED: All ${giftCertPayments.length} gift certificate payments have certificate logs\n`)
    } else {
      console.log(`❌ FAILED: ${missingCertLogs} gift certificate payments missing certificate logs\n`)
    }
