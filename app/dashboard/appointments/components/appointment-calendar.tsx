  ...
  return (
    <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
        ...
        {timeSlots.map((slot, index) => {
          ...
          return (
            <div key={index} className="flex hover:bg-gray-50">
              {/* Time Label */}
              <div className="w-24 flex-shrink-0 border-r border-gray-200 p-4 text-sm text-gray-500">
      ...
      {appointments.length === 0 && (
        <div className="p-8 text-center text-gray-500">No appointments scheduled for this day</div>
      )}
    </div>
  );
}
...
function WeekView({
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
        ...
        {weekDays.map((day, index) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`mt-1 text-2xl font-bold ${isToday ? "text-blue-600" : "text-gray-900"}`}
              >
                {day.getDate()}
              </div>
            </div>
      ...
      {/* Appointment Grid */}
      <div className="grid min-h-[500px] grid-cols-7 divide-x divide-gray-200">
        {weekDays.map((day, index) => {
          const dayKey = day.toDateString();
          ...
          return (
            <div key={index} className="space-y-2 p-2">
              ...
              {dayAppointments.length === 0 && (
                <div className="mt-8 text-center text-sm text-gray-400">No appointments</div>
              )}
            </div>
          );
        })}
